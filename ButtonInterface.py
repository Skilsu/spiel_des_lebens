...
class ImageButton(ButtonInterface):
    def __init__(self, x, y, width, height, image_path, bg_color, hover_color, active_color, border_color=(0, 0, 0),
        ...
        self.image = pygame.transform.scale(self.image, (self.image_rect.width, self.image_rect.height)).convert_alpha()

        self.border_color = border_color

        # must be None because otherwise is printed on the screen with 0
    ...
    def draw(self, screen):
        ...
        if self.player_number is not None:
            num_text = font.render(str(self.player_number), True, (250, 250, 250))
